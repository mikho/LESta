...
As an automated alternative to the hand edit in step 2 above, `install.sh --prepare-config --yes` appends the same exact line to `named.conf` itself for you (idempotent: a rerun against a conf file that already has the line is a no-op). It is a separate, explicitly opt-in mode, never run implicitly by `--dry-run` or `--apply`. It writes to `named.conf` only, never to `named.conf.local`, for the same reason given in step 3 above.
