...
As an automated alternative to the hand edit in step 2 above, `install.sh --prepare-config --yes` appends the same exact line to `apache2.conf` for you (idempotent: a rerun against a conf file that already has the line is a no-op). It is a separate, explicitly opt-in mode, never run implicitly by `--dry-run` or `--apply`, and `--web-profile` is not required for it.
