...
install_apache_offline_bundle() {
    ...
    if ! out=$(install_offline_bundle_debs "${bundle_dir}"); then
        ...
        apache_fail_health "${EXIT_MUTATION_FAILURE}" dpkg_install_failed "${bundle_dir}" "$(printf '%s' "${out}" | tr '\n' ' ')"
    fi
...
run_node_health_selftest() {
    ...
    if ! run_node_health_selftest_delete "${WEB_APACHE_CAPABILITY}" "${resource_id}" "${payload}" "${delete_idem}" "${delete_corr}"; then
        agent_fail_selftest_with_rollback "${EXIT_HEALTH_FAILURE}" selftest_cleanup_failed "${APACHE_LIVE_DIR}" "self-test create succeeded but the throwaway resource could not be deleted afterward"
    fi
