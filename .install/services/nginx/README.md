...
As an automated alternative to the hand edit in step 2 above, `install.sh --prepare-config --yes` inserts the same exact line into `nginx.conf`'s own `http {}` block for you (idempotent: a rerun against a conf file that already has the line is a no-op). It is a separate, explicitly opt-in mode, never run implicitly by `--dry-run` or `--apply`, and `--web-server` is not required for it. If `nginx.conf` has no top-level `http {` line at all (a non-stock or heavily customized file), `--prepare-config` refuses to guess where to insert the line and fails instead -- add it by hand in that case, per steps 2-3 above.
