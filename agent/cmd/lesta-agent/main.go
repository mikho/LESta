...
// environment redirect a privileged exec to an arbitrary executable.
//
// DovecotConfPath is LESta's own dedicated static config file (mirroring
// nginx.conf's/named.conf's own real, operator-added include-line
// prerequisite): `/etc/dovecot/lesta.conf`, expected to already be
// `!include`d from `/etc/dovecot/dovecot.conf` -- Dovecot's own config
// format assembles cleanly across multiple included files (its real
// packaging already ships `!include_try conf.d/*.conf` by default), so
// this mirrors nginx/bind9's pattern exactly.
//
// EximStaticConfPath is NOT the same shape, despite looking parallel: it
// points straight at `/etc/exim4/exim4.conf` itself, not a separate
// LESta-owned file `.include`d from it. Exim's config format has exactly
// one `begin acl`/`begin routers`/`begin transports`/`begin authenticators`
// section each; Debian's default split-config assembly
// (`/etc/exim4/conf.d/*` -> `update-exim4.conf` -> `/var/lib/exim4/
// config.autogenerated`) already defines all four, so `.include`ing a
// second, separate ACL/router/transport/authenticator file into that would
// produce duplicate-section errors, not a working config. The mail
// installer (.install/services/mail/install.sh) resolves this by setting
// `dc_use_split_config='false'` and writing the complete config directly
// to exim4.conf itself -- confirmed by validate.go's own `exim -C
// EximStaticConfPath -bV` self-check, which validates a COMPLETE,
// self-contained config, exactly matching this shape (and exactly
// matching harness_test.go's own disposable-Exim proof, which was always a
// single combined file, not two).
//
// Reload is deliberately left at its own default (SIGHUP to EximPIDFile;
// `doveadm reload` for Dovecot) rather than a `systemctl reload` override:
// the mail installer enables real systemd units for both, but SIGHUP/
// doveadm reload work identically whether or not a unit file exists,
// matching every other capability's own reload-is-orthogonal-to-install
// precedent.
func mailProductionConfig() mail.Config {
	return mail.Config{
		EximStaticConfPath: "/etc/exim4/exim4.conf",
