...
// sidecar is how the nginx capability can later recover *which* domain/aliases/
// ip_address/suspended state a stored generation represents, without needing to
// scrape it back out of rendered nginx config text. It is only ever read for a
...
func (c *NginxCapability) generationMetaPath(resourceID string, n int) string {
	return filepath.Join(c.store.GenerationDir(resourceID, n), generationMetaFileName)
}

func (c *NginxCapability) writeGenerationMeta(resourceID string, n int, payload Payload) error {
...
func (c *NginxCapability) readGenerationMeta(resourceID string, n int) (Payload, error) {
