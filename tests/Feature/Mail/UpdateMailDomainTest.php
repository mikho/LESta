...
test('dkim_enabled is never accepted by update, regardless of what is requested', function () {
    $node = Node::factory()->create();
    NodeCapability::factory()->for($node)->create(['capability' => 'mail.smtp-imap.v1']);
    $mailDomain = MailDomain::factory()->for($node)->create(['dkim_enabled' => false]);
    $owner = Membership::factory()->for($mailDomain->account)->owner()->create()->user;

    $updated = app(UpdateMailDomain::class)->handle($owner, $mailDomain, ['dkim_enabled' => true]);

    expect($updated->dkim_enabled)->toBeFalse();
});
