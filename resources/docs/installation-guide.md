...
- **The LESta control-plane app already running somewhere reachable over HTTPS.** The node's agent authenticates back to it with a bearer token, not mutual TLS (Chapter 3) — the control-plane URL you give the node **must** actually terminate real HTTPS, or the enrollment token and the long-lived credential it exchanges for both travel in plaintext. Not set up yet? `install-cp.sh`, at the repository root (see the repository's own README), walks through it: `.env`, dependencies, a production build, and migrations. In the ADR's own default single-node deployment, the control-plane app and this node's own MariaDB installation (Chapter 4) share the same physical machine — `install-cp.sh` picks up that installer's real generated database credentials automatically when they're present, instead of asking for them.
